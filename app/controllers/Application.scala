...
/*  def headers = List(
  ...
  def query = Action { implicit request =>
    ...
    DB.withConnection { implicit c =>
      val e = SQL("SELECT * FROM ace_metadata WHERE "+AnormHelper.dynIntersectBuilder(params)).
        on(params.map(AnormHelper.dynQueryToNamedParam(_)).toSeq:_*).apply
      Ok(structureQueryOutput(e))
    }
  }
  ...
  def bulkQuery() = Action { implicit request =>
    /*// v = Security Vector (key from the asking system) since this
    // call as the potential to be very heavy.
    // d = Unix timestamp to pull data since. Try not to always pull 0
    val params = request.queryParams
    DB.withConnection { implicit c =>
      val q = SQL("SELECT * FROM ace_metadata WHERE created > {ts}").on('ts -> d).apply
      Ok(structureQueryOutput(q))
    }*/
    Ok("Done.")
  }
}
